...
jQuery(function () {
    window.aeidn_tb_remove = window.tb_remove;
    ...
    jQuery("#aeidn-do-filter").click(function () {
        jQuery("#aeidn-search-form").find("#reset").val("1");
        jQuery("#aeidn-search-form").submit();
        return true;
    });


    jQuery("#aeidn-search-form").submit(function () {
    ...
    jQuery("#aeidn-search-form #current-page-selector").keypress(function (e) {
        if (e.which == 13) {
            jQuery(this).closest('form').submit();
        }
    });

    jQuery("#aeidn-goods-table").on("click", ".select_image", function () {
        return true;
    });

    jQuery("body").on("click", ".aeidn_select_image img", function () {
        ...
        jQuery("#aeidn-goods-table").find('tr').each(function () {
        ...
        var data = {'action': 'aeidn_select_image', 'id': id, 'image': new_image};
    ...
    jQuery("#aeidn-goods-table").on("click", ".edit_btn", function () {
    ...
    jQuery("#aeidn-goods-table").on("click", ".save_btn", function () {
    ...
    jQuery("#aeidn-goods-table").on("click", ".cancel_btn", function () {
        var block = jQuery(this).parents(".block_field");
    ...
    jQuery("#aeidn-goods-table").on("click", ".moredetails", function () {
        ...
        var data = {'action': 'aeidn_load_details', 'id': id, 'edit_fields': edit_fields};
    ...
    jQuery("#aeidn-goods-table").on("click", ".post_import", function () {
        var id = jQuery(this).parents("tr").attr('id');
        var curr_row = jQuery(this).parents("tr");
        var block = jQuery(this).parent();
        ...
        var data = {'action': 'aeidn_import_goods', 'id': id, 'edit_fields': edit_fields};
    ...
    jQuery("#aeidn-goods-table").on("click", "#doaction,#doaction2", function () {
        ...
        jQuery("#aeidn-goods-table .import_process_loader").html("");
        if (check_action == 'import' || check_action == 'import_draft') {
            var num_to_import = jQuery("#aeidn-goods-table input.gi_ckb:checked").length;

            if (num_to_import > 0) {
                jQuery("#aeidn-goods-table .import_process_loader").html("Process import 0 of " + num_to_import + ".");
                var import_cnt = 0;
                var import_error_cnt = 0;
                var import_cnt_total = 0;

                var products_to_import = [];
                jQuery("#aeidn-goods-table input.gi_ckb:checked").each(function () {
                    var id = jQuery(this).parents("tr").attr('id');
                    var curr_row = jQuery(this).parents("tr");
                    ...
                    if (SYNCHRONIZE_IMPORT) {
                        ...
                        jQuery.post(ajaxurl, data, function (response) {
                            ...
                            import_cnt_total++;
                            jQuery("#aeidn-goods-table .import_process_loader").html("Process import " + import_cnt + " of " + num_to_import + ". Errors: " + import_error_cnt + ".");

                            if (import_cnt_total == num_to_import) {
                                jQuery("#aeidn-goods-table .import_process_loader").html("Complete! Result imported: " + import_cnt + "; errors: " + import_error_cnt + ".");
                            }
                        });
                    }
                });
                ...
                if (SYNCHRONIZE_IMPORT) {
                    var cur_index = 0;
                    aeidn_js_post_to_woocomerce(products_to_import, cur_index, import_cnt, import_error_cnt, import_cnt_total);
                }
            }
        } else if (check_action == 'blacklist') {
            var num_to_import = jQuery("#aeidn-goods-table input.gi_ckb:checked").length;
            jQuery("#aeidn-goods-table .import_process_loader").html("Process blacklist 0 of " + num_to_import + ".");
            var import_cnt = 0;
            var import_error_cnt = 0;
            var import_cnt_total = 0;
            ...
            jQuery("#aeidn-goods-table input.gi_ckb:checked").each(function () {
                ...
                jQuery.post(ajaxurl, data, function (response) {
                    ...
                    import_cnt_total++;
                    jQuery("#aeidn-goods-table .import_process_loader").html("Process blacklist " + import_cnt + " of " + num_to_import + ". Errors: " + import_error_cnt + ".");

                    if (import_cnt_total == num_to_import) {
                        jQuery("#aeidn-goods-table .import_process_loader").html("Complete! Result blacklisted: " + import_cnt + "; errors: " + import_error_cnt + ".");
                    }
                });
            });
        }
    ...
    jQuery("#aeidn-goods-table-blacklist").on("click", "#doaction,#doaction2", function () {
        var check_action = (jQuery(this).attr('id') == 'doaction') ? jQuery('#aeidn-goods-table-blacklist #bulk-action-selector-top').val() : jQuery('#aeidn-goods-table-blacklist #bulk-action-selector-bottom').val();

        console.log('check_action ' + check_action);

        if (check_action == 'unblacklist') {
            var num_to_import = jQuery("#aeidn-goods-table-blacklist input.gi_ckb:checked").length;
            jQuery("#aeidn-goods-table-blacklist .import_process_loader").html("Process blacklist 0 of " + num_to_import + ".");
            var import_cnt = 0;
            var import_error_cnt = 0;
            var import_cnt_total = 0;

            jQuery("#aeidn-goods-table-blacklist input.gi_ckb:checked").each(function () {
                var id = jQuery(this).val();
                var curr_row = jQuery(this).parents("tr");
                var block = jQuery(this).parents("tr").find('.row-actions .import');

                var data = {
                    'action': 'aeidn_unblacklist',
                    'id': id
                };
                jQuery.post(ajaxurl, data, function (response) {
                    //console.log('response: ', response);
                    ...
                    import_cnt_total++;
                    jQuery("#aeidn-goods-table-blacklist .import_process_loader").html("Process blacklist " + import_cnt + " of " + num_to_import + ". Errors: " + import_error_cnt + ".");

                    if (import_cnt_total == num_to_import) {
                        jQuery("#aeidn-goods-table-blacklist .import_process_loader").html("Complete! Result blacklisted: " + import_cnt + "; errors: " + import_error_cnt + ".");
                    }
                });
            });
        }
    ...
    function aeidn_js_post_to_woocomerce(products_to_import, cur_index, import_cnt, import_error_cnt, import_cnt_total) {
        if (products_to_import.length > 0 && products_to_import.length > cur_index) {
            ...
            jQuery.post(ajaxurl, products_to_import[cur_index], function (response) {
                ...
                aeidn_js_post_to_woocomerce(products_to_import, cur_index + 1, import_cnt, import_error_cnt, import_cnt_total);
            });
        }
    }
    ...
    jQuery(".schedule_post_date").datetimepicker({
        ...
        onSelectTime: function (dateText, input) {
            ...
            var data = {'action': 'aeidn_schedule_import_goods', 'id': id, 'time': jQuery(input).val()};
    ...
    jQuery("#aeidn-goods-table").on("click", ".schedule_post_import", function () {
        jQuery(this).prev().datetimepicker('show');
        return false;
    });
    ...
    jQuery(".edit_desc_action").click(function () {
        ...
        var data = {'action': 'aeidn_description_editor', 'id': id};
        jQuery.post(ajaxurl, data, function (response) {
            //console.log('response: ', response);
    ...
    jQuery("#aeidn_add_formula").click(function () {
        var this_row = jQuery(this).parents('tr');

        var data = {
            'action': 'aeidn_price_formula_add',
            'type': jQuery('#aeidn_price_formula_add_form [name="type"]').val(),
            'type_name': jQuery('#aeidn_price_formula_add_form [name="type"] option:selected').text(),
            'category': jQuery('#aeidn_price_formula_add_form [name="category"]').val(),
            'category_name': jQuery('#aeidn_price_formula_add_form [name="category"] option:selected').text(),
            'min_price': jQuery('#aeidn_price_formula_add_form [name="min_price"]').val(),
            'max_price': jQuery('#aeidn_price_formula_add_form [name="max_price"]').val(),
            'sign': jQuery('#aeidn_price_formula_add_form [name="sign"]').val(),
            'value': jQuery('#aeidn_price_formula_add_form [name="value"]').val(),
            'discount1': jQuery('#aeidn_price_formula_add_form [name="discount1"]').val(),
            'discount2': jQuery('#aeidn_price_formula_add_form [name="discount2"]').val()
        };
        ...
        jQuery('#aeidn_price_formula_add_form input').val('');
        jQuery("#aeidn_price_formula_add_form select").prop("selectedIndex", 0);
        ...
        jQuery.post(ajaxurl, data, function (response) {
            ...
            jQuery('#aeidn_price_formula').append('<tr formula-id="' + json.formula.id + '"><td>' + json.formula.pos + '</td><td>' + json.formula.type_name + '</td><td>' + json.formula.category_name + '</td><td>' + json.formula.min_price + ' < PRICE < ' + json.formula.max_price + '</td><td>' + ((json.formula.sign == "=") ? (json.formula.value) : ("PRICE " + json.formula.sign + " " + json.formula.value)) + '</td><td>' + discount_html + '</td><td><button class="button-primary aeidn_edit_formula">Edit</button> <button class="button-primary aeidn_del_formula">Delete</button></td></tr>');

            jQuery('#aeidn_price_formula_add_form button').removeAttr('disabled');
    ...
    jQuery("#aeidn_price_formula").on("click", ".aeidn_edit_formula", function () {
        var this_row = jQuery(this).parents('tr');
        var data = {'action': 'aeidn_price_formula_get', 'id': jQuery(this_row).attr('formula-id')};
        ...
        jQuery.post(ajaxurl, data, function (response) {
            ...
            html += '<td class="action"><a class="button-primary aeidn_save_formula">Save</a> <a class="button-primary aeidn_cancel_edit_formula">Cancel</a></td>';
    ...
    jQuery("#aeidn_price_formula").on("click", ".aeidn_save_formula", function () {
        var this_row = jQuery(this).parents('tr');
        var data = {
            'action': 'aeidn_price_formula_edit',
    ...
    jQuery("#aeidn_price_formula").on("click", ".aeidn_cancel_edit_formula", function () {
        var this_row = jQuery(this).parents('tr');
        jQuery(this_row).prev().show();
        jQuery(this_row).remove();
    });

    jQuery("#aeidn_price_formula").on("click", ".aeidn_del_formula", function () {
        var this_row = jQuery(this).closest('tr');
        var data = {'action': 'aeidn_price_formula_del', 'id': jQuery(this_row).attr('formula-id')};
        ...
        jQuery.post(ajaxurl, data, function (response) {
            ...
            var find = false;
            jQuery('#aeidn_price_formula tr').each(function (index, el) {
                if (find) {
